...
#define			Vertical	1  	//Portrait
#define			Apaisada	0 	//landscape

#define LCD_PIXEL_HEIGHT          0x0140
#define LCD_PIXEL_WIDTH         0x00F0

//#define ASSEMBLE_RGB(R ,G, B)    ((((R)& 0xF8) << 8) | (((G) & 0xFC) << 3) |
				//(((B) & 0xF8) >> 3))
...
void LCD_Rectangle(uint16_t x, uint16_t y, uint16_t longx, uint16_t longy,
		   uint8_t grosor, uint16_t color);
void LCD_RectangleFill(uint16_t x, uint16_t y, uint16_t longx, uint16_t longy,
		       uint16_t color);
...
void LCD_WriteBMP(uint8_t Xpos, uint16_t Ypos, uint8_t Height, uint16_t Width,
		  uint8_t *bitmap);
void LCD_DrawFullSquare(uint16_t Xpos, uint16_t Ypos, uint16_t a);

void LCD_Button(uint16_t x, uint16_t y, uint16_t longx, uint16_t longy, uint8_t p,
		uint8_t* text, uint16_t ink, uint16_t bkcolor);
